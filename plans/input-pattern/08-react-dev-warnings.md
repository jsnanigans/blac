...
If the caller passes BOTH an explicit `instanceId` and `args` whose derived key differs from that `instanceId`, warn that the explicit id overrides the derived identity (usually a mistake). (Core task 03 warns on same-key arg *mismatch*; this is the React-side "you keyed two ways" warning.)
...
it('select replaces dependencies for manual deps', () => { /* assert select still drives manual-deps mode */ });
```
