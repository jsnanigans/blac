...
class UserCard extends Cubit<{ id: string | null }, { userId: string }> {
  ...
  protected init(a: { userId: string }) { this.emit({ id: a.userId }); }
}
it('seeds state from args; different userId → different instance', () => {
  const { rerender, result } = renderHook(({ u }) => useBloc(UserCard, { args: { userId: u } }), { initialProps: { u: 'a' } });
  expect(result.current[0].id).toBe('a');
  rerender({ u: 'b' });
  expect(result.current[0].id).toBe('b'); // re-resolved to the 'b' instance
});
it('same userId across two consumers shares one instance', () => { /* mount two, assert bloc identity */ });
