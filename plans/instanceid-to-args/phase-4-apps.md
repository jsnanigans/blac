...
- `acquire(CounterBloc, key,…)` / `release(CounterBloc, \`bench-${i}\`,…)` use programmatic string
  keys. Give the benchmark `CounterBloc` a `static key = (a)=>a.id` + `Args={id}` and pass `{ args }`
  (and `{ args, refId }` for acquire / `{ args, refId }` for release). Keep benchmark semantics.
