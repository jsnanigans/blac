...
export class NotificationStage extends PipelineStage {
  ...
  private throttleNotification<T>(context: PipelineContext<T>): void {
    ...
    if (timeSinceLastCall >= this.options.throttleMs) {
      ...
      BlacLogger.debug(
        'NotificationStage',
        '⚡ Throttle allowed - sending notification',
        {
          subscriptionId: context.subscriptionId,
          timeSinceLastCall,
          throttleMs: this.options.throttleMs,
      ...
      if (!this.throttleTimer) {
        ...
        BlacLogger.debug(
          'NotificationStage',
          '⚡ Throttle blocked - scheduling trailing call',
          {
            subscriptionId: context.subscriptionId,
            timeSinceLastCall,
            remainingTime,
        ...
        BlacLogger.debug(
          'NotificationStage',
          '⚡ Throttle blocked - trailing call already scheduled',
          {
            subscriptionId: context.subscriptionId,
            explanation:
              'Within throttle window and a trailing call is already scheduled',
          },
        );
      }
    }
