  ...
  static async waitForState<T extends BlocBase<S>, S>(
    ...
    return new Promise<S>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        reject(new Error(`Timeout waiting for state matching predicate after ${timeout}ms`));
      }, timeout);
 ...
 */
export class MockBloc<S, A extends BlocEventConstraint = BlocEventConstraint> extends Bloc<S, A> {
  private mockHandlers = new Map<string, (event: A, emit: (newState: S) => void) => void | Promise<void>>();
