...
describe('Schema Validation - ArkType Integration', () => {
  ...
  describe('Cubit Helper Methods', () => {
    const NumberSchema = type('number>=0').atMost(100);
  ...
  describe('Vertex (Bloc) with Schema', () => {
    const CounterStateSchema = type({
      count: type('number>=0').narrow((n) => Number.isInteger(n)),
      label: 'string',
    });

    type CounterState = typeof CounterStateSchema.infer;
    ...
    it('should validate state on event (failure - negative count)', async () => {
      // Event handlers are async and errors may be handled internally
      try {
        await bloc.add(new IncrementEvent(-10));
      } catch (error) {
        // Error might be thrown or handled internally
      }
      // State should be unchanged regardless
      expect(bloc.state.count).toBe(0);
    });

    it('should validate state on event (failure - non-integer)', () => {
      // ArkType's narrow() validation with integer check
      // Save original state
  ...
  describe('Cubit with Coercion Schema', () => {
    // ArkType can morph types using .pipe() with transformations
    // Use 'string | number' for union, not ['string', 'number'] which is a tuple
    const CoercionSchema = type('string|number').pipe((value) => String(value));
