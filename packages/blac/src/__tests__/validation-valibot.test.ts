...
describe('Schema Validation - Valibot Integration', () => {
  describe('Cubit with Simple Schema', () => {
    ...
    it('should NOT validate on regular emit', () => {
      ...
      cubit.setValue(-1); // Below min, but no validation
  ...
  describe('Cubit Helper Methods', () => {
    const NumberSchema = v.pipe(v.number(), v.minValue(0), v.maxValue(100));
  ...
  describe('Vertex (Bloc) with Schema', () => {
    const CounterStateSchema = v.object({
      count: v.pipe(v.number(), v.integer(), v.minValue(0)),
      label: v.string(),
    });

    type CounterState = v.InferOutput<typeof CounterStateSchema>;
  ...
  describe('Cubit with Coercion Schema', () => {
    ...
    class CoercionCubit extends Cubit<number> {
      ...
      setValue = (value: number | string) => {
        this.emit(value as number);
      };

      setValueValidated = (value: number | string) => {
        this.emitValidated(value as number);
      };
    }
    ...
    it('should handle schema coercion with emitValidated', () => {
      ...
      // String should be coerced to number with validation
      cubit.setValueValidated('42');
      expect(cubit.state).toBe(42);
      expect(typeof cubit.state).toBe('number');
