...
export class SubscriptionManager<TBloc extends BlocBase<any>> {
  ...
  notifySubscribers(
    ...
    for (const [consumerId, { listener, selector, consumerRef }] of this.subscriptions) {
      ...
      if (selector) {
        ...
        shouldNotify = this.consumerTracker.shouldNotifyConsumer(consumerRef, changedPaths);
      }
