...
describe('StateContainer subscriptions', () => {
  ...
  it('listener receives updated state on emit', async () => {
    const container = new SimpleContainer();
  ...
  it('multiple listeners all receive state update', async () => {
    const container = new SimpleContainer();
    const a = vi.fn();
    const b = vi.fn();
    const c = vi.fn();
  ...
  it('listener errors are caught and logged — other subscribers still called', async () => {
    const container = new SimpleContainer();
  ...
  it('unsubscribing during emission is safe', async () => {
    const container = new SimpleContainer();
  ...
  it('listener added inside another listener — new listener not called in current emit', async () => {
    const container = new SimpleContainer();
  ...
  it('same listener function added twice — only stored once (Set semantics)', async () => {
    const container = new SimpleContainer();
    const listener = vi.fn();
    container.subscribe(listener);
    container.subscribe(listener);
