...
interface ExternalStoreOptions<B extends BlocBase<any>> {
  ...
  selector?: (currentState: BlocState<B>, previousState: BlocState<B>, instance: B) => any[];
}
 ...
 */
export default function useExternalBlocStore<B extends BlocConstructor<BlocBase<any>>>(
  blocConstructor: B,
  ...
  const externalStore = useMemo<ExternalStore<BlocState<InstanceType<B>>>>(() => ({
    ...
    subscribe: (listener: () => void) => {
      ...
      const safeListener = (newState: BlocState<InstanceType<B>>, oldState: BlocState<InstanceType<B>>) => {
        try {
