...
describe('useExternalBlocStore', () => {
  ...
  describe('Store Creation', () => {
    ...
    it('should create isolated bloc instance when isolated flag is set', () => {
      const { result: result1 } = renderHook(() =>
        useExternalBlocStore(IsolatedTestCubit),
      );
      const { result: result2 } = renderHook(() =>
        useExternalBlocStore(IsolatedTestCubit),
      );

      expect(result1.current.instance.current).not.toBe(
        result2.current.instance.current,
      );
      expect(result1.current.instance.current?._id).not.toBe(
        result2.current.instance.current?._id,
      );
    });
  });
  ...
  describe('Store Consistency', () => {
    it('should return same bloc instance for same constructor', () => {
      const { result: result1 } = renderHook(() =>
        useExternalBlocStore(TestCubit),
      );
      const { result: result2 } = renderHook(() =>
        useExternalBlocStore(TestCubit),
      );

      expect(result1.current.instance.current).toBe(
        result2.current.instance.current,
      );
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should return undefined when no instance exists', () => {
      ...
      expect(
        result.current.externalStore.getServerSnapshot?.(),
      ).toBeUndefined();
    });
  });
});
