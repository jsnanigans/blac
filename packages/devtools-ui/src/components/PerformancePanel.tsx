...
type SortKey = 'updatesPerSecond' | 'stateSizeBytes' | 'totalUpdates' | 'maxBurstRate';
...
function warningColor(warnings: InstanceMetrics['warnings']): string {
  if (warnings.length === 0) return 'transparent';
  return '#ef4444';
}

const SparkBar: FC<{ value: number; max: number; color: string }> = ({ value, max, color }) => {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: '10px', color: '#888', minWidth: '28px', textAlign: 'right' }}>
        {value.toFixed(1)}
      </span>
    </div>
  );
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {warnings.length > 0 && (
        ...
        >
          <span style={{ color: '#ef4444', fontWeight: 600, marginRight: '8px' }}>
        ...
        >
          <thead>
            ...
            >
              ...
              <th
                style={{ padding: '6px 10px', textAlign: 'left', ...colStyle('updatesPerSecond') }}
                onClick={() => handleSort('updatesPerSecond')}
              >
                Updates/sec {sortKey === 'updatesPerSecond' ? (sortDesc ? '↓' : '↑') : ''}
              </th>
              <th
                style={{ padding: '6px 10px', textAlign: 'left', ...colStyle('totalUpdates') }}
                onClick={() => handleSort('totalUpdates')}
              >
                Total {sortKey === 'totalUpdates' ? (sortDesc ? '↓' : '↑') : ''}
              </th>
              <th
                style={{ padding: '6px 10px', textAlign: 'left', ...colStyle('maxBurstRate') }}
                onClick={() => handleSort('maxBurstRate')}
              >
                Peak burst {sortKey === 'maxBurstRate' ? (sortDesc ? '↓' : '↑') : ''}
              </th>
              <th
                style={{ padding: '6px 10px', textAlign: 'left', ...colStyle('stateSizeBytes') }}
            ...
            {visibleRows.map((m) => {
              ...
              return (
                <tr
                  ...
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.background = hasWarning
                      ? '#261818'
                      : '#1e1e1e';
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <td style={{ padding: '6px 10px' }}>
                    <SparkBar
                      ...
                      max={maxUps}
                      color={m.updatesPerSecond > 30 ? '#ef4444' : m.updatesPerSecond > 10 ? '#FFB74D' : '#10b981'}
                    />
                  </td>
                  ...
                  <td style={{ padding: '6px 10px' }}>
                    <SparkBar
                      ...
                      max={maxSize / 1024}
                      color={m.stateSizeBytes > 102400 ? '#ef4444' : m.stateSizeBytes > 10240 ? '#FFB74D' : '#569cd6'}
                    />
...
const UpdateTimeline: FC<{ metrics: InstanceMetrics[] }> = React.memo(({ metrics }) => {
  ...
  return (
    ...
    >
      ...
      {activeMetrics.map((m) => {
        const timestamps = metricsState.updateTimestamps.get(m.instanceId) || [];
        const buckets = Array(BUCKETS).fill(0);
        for (const t of timestamps) {
          const age = now - t;
          if (age < WINDOW) {
            const bucketIdx = Math.floor(age / 1000);
            buckets[BUCKETS - 1 - bucketIdx] = (buckets[BUCKETS - 1 - bucketIdx] || 0) + 1;
          }
        }
        ...
        return (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '1px', flex: 1, height: '16px', alignItems: 'flex-end' }}>
              {buckets.map((count, i) => (
                <div
                  key={i}
                  style={{
                  ...
                  title={count > 0 ? `${count} update${count !== 1 ? 's' : ''}` : undefined}
                />
              ))}
            </div>
          </div>
