...
describe('PersistencePlugin', () => {
  ...
  describe('Basic Persistence', () => {
    ...
    it('should handle complex state objects', async () => {
      ...
      plugin.onStateChange(
        ...
        { ...cubit.state, name: 'Jane', preferences: { ...cubit.state.preferences, theme: 'dark' } }
      );
  ...
  describe('Migrations', () => {
    it('should migrate data from old keys', async () => {
      // Set old data
      storage.setItem('old-user-key', JSON.stringify({
        firstName: 'John',
        lastName: 'Doe',
        age: 30
      }));
  ...
  describe('Encryption', () => {
    it('should encrypt and decrypt stored data', async () => {
      ...
      cubit2.addPlugin(new PersistencePlugin<number>({
        key: 'counter',
        storage,
        encrypt: { encrypt, decrypt }
      }));
  ...
  describe('Clear', () => {
    it('should clear stored state and metadata', async () => {
      ...
      storage.setItem('counter__metadata', JSON.stringify({
        version: 1,
        timestamp: Date.now()
      }));
