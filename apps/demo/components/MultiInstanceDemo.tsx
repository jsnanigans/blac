...
const CounterInstance: React.FC<{ id: string; initialCount?: number }> = ({
  id,
  initialCount,
}) => {
  ...
  return (
    <div className="p-3 border border-dashed border-muted-foreground rounded-md space-y-2">
      <p className="text-md font-medium">
        Instance "{id}" Count:{' '}
        <span className="text-primary font-bold text-lg">{state.count}</span>
      </p>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground mt-2">
        Each counter above uses the same `CounterCubit` class but is provided a
        unique `instanceId` via `useBloc(CounterCubit, &#123; instanceId:
        'unique-id' &#125;)`. This ensures they maintain separate states.
      </p>
    </div>
  );
};
