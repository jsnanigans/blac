...
const LifecycleComponent: React.FC = () => {
  ...
  return (
    <div className="p-3 border border-dashed border-accent rounded-md space-y-2 bg-accent/10">
      ...
      <p>
        Status:{' '}
        <span className="font-semibold text-primary">{state.status}</span>
      </p>
      <p>
        Data:{' '}
      ...
      <p>
        ...
        <span className="text-xs">
          {state.mountTime ? state.mountTime.toLocaleTimeString() : 'N/A'}
        </span>
      </p>
      <p>
        Unmounted at:{' '}
        <span className="text-xs">
          {state.unmountTime ? state.unmountTime.toLocaleTimeString() : 'N/A'}
        </span>
      </p>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground mt-2">
        The component above uses `onMount` and `onUnmount` callbacks with
        `useBloc`. `onMount` is called when the component mounts and `onUnmount`
        when it unmounts. The `LifecycleCubit` itself is `static isolated =
        true`.
      </p>
    </div>
  );
