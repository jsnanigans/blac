...
const CounterDisplayComponent: React.FC<CounterDisplayProps> = ({ id }) => {
  ...
  return (
    <div
      style={{ border: '1px dashed #999', padding: '1rem', margin: '0.5rem 0' }}
    >
      <h4>Display Component (ID: {id})</h4>
      <p>
        Connected to Cubit Instance ID: <strong>{state.instanceId}</strong>
      </p>
      <p>
        Current Count: <strong>{state.count}</strong>
      </p>
      <Button onClick={cubit.increment} variant="default">
        Increment from Display
      </Button>
    </div>
  );
};
...
const KeepAliveDemo: React.FC = () => {
  ...
  const handleResetGlobalKeepAliveCounter = () => {
    ...
    try {
      ...
      if (cubit) {
        cubit.reset();
        console.log(
          'KeepAliveCounterCubit RESET triggered from parent via Blac.getBloc().',
        );
      } else {
        console.warn(
      ...
      console.error(
        'Error getting KeepAliveCounterCubit via Blac.getBloc()',
        e,
      );
    }
  };
  ...
  const handleIncrementGlobalKeepAliveCounter = () => {
    ...
    console.log(
      'KeepAliveCounterCubit INCREMENT triggered from parent via useBloc instance.',
    );
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <p className="text-sm text-muted-foreground">
        <code>KeepAliveCounterCubit</code> has{' '}
        <code>static keepAlive = true</code>. Its instance persists even if no
        components are using it. (Check console logs for construction/disposal
        messages - disposal won't happen until Blac is reset or a specific
        dispose call).
      </p>

      <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
        <Button
          onClick={() => setShowDisplay1(!showDisplay1)}
          variant="outline"
        >
          {showDisplay1 ? 'Hide' : 'Show'} Display 1
        </Button>
        <Button
          onClick={() => setShowDisplay2(!showDisplay2)}
          variant="outline"
        >
      ...
      {!showDisplay1 && !showDisplay2 && (
        <p style={{ color: 'orange', marginTop: '1rem' }}>
          Both display components are unmounted. The{' '}
          <code>KeepAliveCounterCubit</code> instance should still exist in
          memory with its current state.
        </p>
      )}
      ...
      >
        ...
        <Button
          onClick={handleIncrementGlobalKeepAliveCounter}
          variant="secondary"
        ...
        <Button
          onClick={handleResetGlobalKeepAliveCounter}
          variant="destructive"
        >
      ...
      <p className="text-xs text-muted-foreground mt-2">
        Toggle the display components. When they remount, they should connect to
        the same
        <code>KeepAliveCounterCubit</code> instance and reflect its preserved
        state. The instance ID should remain the same across mounts/unmounts of
        display components. Console logs provide more insight into Cubit
        lifecycle.
      </p>
    </div>
  );
