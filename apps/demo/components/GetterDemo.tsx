...
const GetterDemo: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-base text-foreground">
        Text Length (from getter):{' '}
        <span className="text-primary font-bold font-body normal-case">
          {cubit.textLength}
        </span>
      </p>
      <p className="text-base text-foreground">
        Uppercased Text (from getter):{' '}
        <span className="text-secondary font-bold font-body normal-case">
          <ShowUppercased />
        </span>
      </p>
      ...
      <p className="text-sm text-muted-foreground mt-4">
        This component primarily displays values derived from getters
        (`textLength`, `uppercasedText`). It should re-render when `state.text`
        changes (as the getters depend on it). Changing other parts of
        `ComplexStateCubit` state (like `counter`) should not cause a re-render
        here if those parts are not directly used or via a getter that changes.
      </p>
    </div>
  );
};
