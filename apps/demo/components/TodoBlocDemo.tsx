...
const TodoItem: React.FC<{
  ...
  return (
    <div style={{
      ...
      <Button onClick={() => onRemove(todo.id)} variant="destructive" style={{ padding: '2px 6px', fontSize: '0.8em' }}>
        Remove
      </Button>
    </div>
  );
    ...
    <div style={{ maxWidth: '500px', margin: '0 auto' }}>
      ...
      {state.todos.length > 0 && (
       ...
       <p className="text-xs text-muted-foreground mt-4">
        This demo showcases a <code>Bloc</code> using the new event-handler pattern (<code>this.on(EventType, handler)</code>) to manage a todo list.
        Actions (which are now classes like <code>AddTodoAction</code>, <code>ToggleTodoAction</code>, etc.) are dispatched via <code>bloc.add(new EventType())</code>,
        often through helper methods on the <code>TodoBloc</code> itself (e.g., <code>bloc.addTodo(text)</code>).
        The <code>TodoBloc</code> then processes these events with registered handlers to produce new state.
      </p>
    </div>
  );
};
