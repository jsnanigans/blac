  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      >
        ...
        {authState.isAuthenticated ? (
          <div>
            <p>
              Logged in as:{' '}
              <strong style={{ color: 'green' }}>{authState.userName}</strong>
            </p>
            <Button onClick={authCubit.logout} variant="destructive">
              Logout
            </Button>
          </div>
        ) : (
      ...
      >
        ...
        <Button
          onClick={dashboardStatsCubit.loadDashboard}
          variant="default"
        ...
        <Button
          onClick={dashboardStatsCubit.resetStats}
          variant="outline"
      ...
      <p className="text-xs text-muted-foreground mt-2">
        This demo illustrates communication between Blocs/Cubits. The{' '}
        <code>DashboardStatsCubit</code> (isolated instance) uses{' '}
        <code>Blac.getBloc(AuthCubit)</code> to access the state of the shared{' '}
        <code>AuthCubit</code>. The dashboard stats will reflect the current
