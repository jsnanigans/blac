    ...
    <div style={{ padding: '10px', border: '1px solid #ddd', borderRadius: '4px' }}>
      ...
      {cubit.availableProducts.map(product => (
        <div 
          ...
          style={{ 
            padding: '5px',
            backgroundColor: state.ui.selectedProductId === product.id ? '#e0f0ff' : 'transparent',
            cursor: 'pointer'
          }}
  ...
  const [state] = useBloc(AppStateCubit, {
    selector: (state) => {
      ...
      if (state.user.preferences.theme === 'dark') {
        return [state.user.preferences.theme, state.user.preferences.notifications];
      } else {
  ...
  return (
    <div style={{ padding: '10px', border: '1px solid #ddd', borderRadius: '4px' }}>
      ...
      {state.user.preferences.theme === 'dark' ? (
        <div>Notifications: {state.user.preferences.notifications ? 'ON' : 'OFF'}</div>
      ) : (
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <Button 
              onClick={() => cubit.updateProductPrice('p1', Math.floor(Math.random() * 1000))}
              size="sm"
            >
              Random Laptop Price
            </Button>
            ...
            <Button 
              onClick={() => cubit.toggleProductStock('p3')}
              size="sm"
            >
            ...
            <Button 
              onClick={() => cubit.updateCartQuantity('p1', Math.floor(Math.random() * 5))}
              size="sm"
            >
      ...
      <div style={{ 
        ...
        <ul style={{ marginTop: '10px', paddingLeft: '20px' }}>
          <li><strong>Simple path selector:</strong> <code>[state.user.name]</code> - Only re-renders on name change</li>
          <li><strong>Nested path selector:</strong> <code>[state.user.preferences.theme]</code> - Deep property tracking</li>
          <li><strong>Computed value selector:</strong> Using getters like <code>instance.cartTotal</code></li>
          <li><strong>Multiple dependencies:</strong> <code>[availableProducts, selectedProductId]</code></li>
          <li><strong>Conditional selector:</strong> Different dependencies based on state</li>
          <li><strong>Complex computation:</strong> Transforming state before comparison</li>
        </ul>
        
        <strong style={{ display: 'block', marginTop: '15px' }}>Performance Tips:</strong>
