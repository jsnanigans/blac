...
const DisplayCounter: React.FC = React.memo(() => {
  ...
  return (
    <p>
      Counter: <span className="font-bold text-primary">{state.counter}</span>{' '}
...
const DisplayText: React.FC = React.memo(() => {
  ...
  return (
    <p>
      Text: <span className="font-bold text-secondary">{state.text}</span>{' '}
      (Renders: {renderCountRef.current})
    </p>
  );
});
...
const DisplayFlag: React.FC = React.memo(() => {
  ...
  return (
    <p>
      Flag: <span className="font-bold">{state.flag ? 'TRUE' : 'FALSE'}</span>{' '}
      (Renders: {renderCountRef.current})
    </p>
  );
});
...
const DisplayNestedValue: React.FC = React.memo(() => {
  ...
  return (
    <p>
      Nested Value:{' '}
      <span className="font-bold text-green-600">{state.nested.value}</span>{' '}
      (Renders: {renderCountRef.current})
    </p>
  );
});
...
const DisplayTextLengthGetter: React.FC = React.memo(() => {
  ...
  return (
    <p>
      Text Length (Getter):{' '}
      <span className="font-bold text-purple-600">{cubit.textLength}</span>{' '}
      (Renders: {renderCountRef.current})
    </p>
  );
});
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Button
            onClick={() => cubit.updateText('Reset Text')}
            variant="outline"
            size="sm"
          >
            Reset Text
      ...
      <p className="text-xs text-muted-foreground mt-2">
        Each displayed piece of state above is in its own component that
        subscribes to only that specific part of the `ComplexDemoState` (or a
        getter). Only the component whose subscribed state (or getter result)
        changes should re-render.
      </p>
    </div>
  );
};
