...
class StreamDemoCubit extends Cubit<{ count: number; message: string; flag: boolean }> {
  constructor() {
...
const StreamApiDemo: React.FC = () => {
  ...
  const [messageLength, setMessageLength] = useState(cubit.state.message.length);
  const [subscriptionLogs, setSubscriptionLogs] = useState<string[]>([]);

  useEffect(() => {
    const addLog = (message: string) => {
      const timestamp = new Date().toLocaleTimeString();
      setSubscriptionLogs(prev => [...prev.slice(-4), `[${timestamp}] ${message}`]);
    };
    ...
    const unsubscribe4 = cubit.subscribeWithSelector(
      ...
      (selected) => {
        addLog(`Count or flag changed: count=${selected.count}, flag=${selected.flag}`);
      }
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <Button onClick={cubit.incrementCount}>
            Increment Count
          </Button>
          ...
          <Button onClick={cubit.toggleFlag}>
            Toggle Flag
          </Button>
      ...
      <div style={{ 
        ...
        <ul style={{ marginTop: '5px', paddingLeft: '20px' }}>
          <li><code>subscribe(callback)</code> - Receives all state changes</li>
          <li><code>subscribeWithSelector(selector, callback)</code> - Only triggers when selected value changes</li>
          <li><code>subscribeComponent(weakRef, callback)</code> - Component-safe subscription (used internally by hooks)</li>
        </ul>
        <strong style={{ display: 'block', marginTop: '10px' }}>Note:</strong> 
        Subscriptions must be cleaned up to prevent memory leaks. All methods return an unsubscribe function.
      </div>
    </div>
  );
};
