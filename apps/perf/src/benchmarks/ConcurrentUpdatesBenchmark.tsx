...
const SharedCounterComponent = forwardRef<ComponentHandle, { id: number }>(
  ({ id }, ref) => {
...
const IsolatedCounterComponent = forwardRef<ComponentHandle, { id: number }>(
  ({ id }, ref) => {
...
export const ConcurrentUpdatesBenchmark: React.FC = () => {
  ...
  const Component = useIsolated
    ? IsolatedCounterComponent
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <p>
        Tests how BlaC handles many components updating simultaneously. Measures
        batching, subscription performance, and re-render optimization.
      </p>
      ...
      >
        ...
        <div style={{ fontSize: '14px', lineHeight: '1.6' }}>
          <p style={{ marginBottom: '10px' }}>
            <strong>Shared Mode:</strong> All {componentCount} components
            subscribe to ONE shared state. Each update triggers re-renders in
            all components (1 update → {componentCount} re-renders).
          </p>
          <p style={{ marginBottom: 0 }}>
            <strong>Isolated Mode:</strong> Each component has its OWN isolated
            state instance. Updates only affect the specific component (1 update
            → 1 re-render).
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: '12px', color: '#666', marginTop: '8px' }}>
          ...
          {useIsolated
            ...
            : `All ${componentCount} components share one state`}
        </div>
      </div>
        ...
        <div>
          ...
          <div
            style={{
              marginBottom: '15px',
              padding: '12px',
              background: useIsolated ? '#f3e5f5' : '#e3f2fd',
              borderRadius: '4px',
            }}
          >
          ...
          >
            ...
            <tbody>
              {results.map((result, i) => {
                ...
                const expected =
                  results.find((r) => r.name === 'Expected Re-renders')
                    ?.iterations || 0;
                const isActualRow =
                  result.name === 'Total Component Re-renders';
                const renderEfficiency =
                  isActualRow && expected > 0
                  ...
                  <tr key={i}>
                    ...
                    >
                      {isActualRow && renderEfficiency && (
                        <span
                          style={{
                            color: actual === expected ? '#4caf50' : '#ff9800',
                          }}
                        >
                          {actual === expected
                            ? '✓ Exact match'
                            : `${renderEfficiency}% of expected`}
                        </span>
                      )}
          ...
          >
            ...
            <div style={{ marginTop: '5px', lineHeight: '1.6' }}>
              {useIsolated ? (
                <>
                  <div>
                    • Isolated mode: Each component manages its own state
                    independently
                  </div>
                  <div>
                    • Total work: {componentCount} × 100 ={' '}
                    {componentCount * 100} state updates
                  </div>
                  <div>
                    • Each update only re-renders the specific component that
                    changed
                  </div>
                </>
              ) : (
                <>
                  <div>
                    • Shared mode: All components subscribe to the same state
                  </div>
                  <div>• Total work: 100 state updates to 1 shared cubit</div>
                  <div>
                    • Each update potentially triggers {componentCount}{' '}
                    component re-renders
                  </div>
                  <div>
                    • React batching may reduce actual re-renders from
                    theoretical {componentCount * 100}
                  </div>
                </>
              )}
            </div>
