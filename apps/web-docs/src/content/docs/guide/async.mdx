...
`useBloc` never throws a promise to a Suspense boundary or unwraps one via `use()`. There is no code path in `useBloc` that suspends a component.
...
A request in flight when the bloc is disposed will try to `emit` on a dead container. That no longer throws — it's a no-op (with a dev-only console warning) — but the update is silently dropped, which wastes the in-flight work. Wire `onSystemEvent('dispose', …)` to abort the controller so the request unwinds cleanly:
...
- [useBloc](/react/use-bloc) — `onMount`, `select`, and how re-renders are scoped
- [System Events](/core/system-events) — the `dispose` and `hydrationChanged` lifecycle hooks
- [Core Concepts](/guide/concepts/) — the vocabulary and model behind these APIs
