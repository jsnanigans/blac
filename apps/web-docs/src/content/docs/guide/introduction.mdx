---
title: What is BlaC?
description: BlaC is a TypeScript state management library for React that separates business logic into class-based, auto-tracked state containers.
---
...
BlaC (Business Logic Components) is a TypeScript state management library for React. It separates business logic into class-based state containers — [Cubits](/guide/concepts/#the-core-model) — that are type-safe, testable, and automatically optimized for minimal re-renders.
...
The short version: BlaC keeps logic in classes (like flutter_bloc), shares instances through a ref-counted registry (no providers), and tracks re-renders with a render-time proxy (no selectors or `useMemo`).
:::
