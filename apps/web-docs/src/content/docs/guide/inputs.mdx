...
The three lanes — `args`, `deps`, and events — and the [identity model](#identity-model-and-precedence) below are the whole story. For the broader _why_ behind shared, ref-counted instances see the [Mental Model](/guide/mental-model),.
...
class UserCardCubit extends Cubit<UserCardState, { userId: string }> {
  ...
  // it only fires once something actually owns the instance. See Cubit's
  // onActivate / onDeactivate docs.
...
See [Configuration](/core/configuration) for the rest of the `blac()` options (`keepAlive`, `equality`, `excludeFromDevTools`) and [Core Concepts](/guide/concepts/) for how `args` and `static key` relate.
...
There is no `deps:` key on `useBloc`. A component contributes its slice by calling `useBlocDeps(bloc, slice)`, exported alongside `useBloc`:
:::
...
function UploadButton({ endpoint }: { endpoint: string }) {
  ...
  useBlocDeps(cubit, { inputRef }); // contribute this consumer's slice

  return <button onClick={() => cubit.openPicker()}>Choose file</button>;
}
```

Each calling component is one owner: `useBlocDeps` tags its slice internally (with a stable per-mount id) so the engine knows which consumer wrote it, shallow-merges it into the bloc's merged `deps` view, and withdraws exactly that slice automatically on unmount. No dependency array is needed — the hook applies the slice on every commit and the core shallow-compares, so an unchanged slice is a no-op; still pass identity-stable values (`useMemo` / `useCallback`), since a new function identity counts as a change.

<details>
<summary>Why an effect, not a render-time call</summary>

Deps must be applied _after_ commit so a freshly mounted `ref.current` is populated and so mid-render mutation never happens. `useBlocDeps` applies in an effect, post-commit; its cleanup runs before the consumer's `useBloc` releases its ref, so the slice is withdrawn while the bloc is still alive.
...
- **Reaching for `deps:`, `instanceId:`, or `autoInstance:` as `useBloc` options** — none exist. Wire deps with `useBlocDeps`; key per-mount instances with a synthetic `args` value + `static key` (e.g. `args: { _id: useId() }`).
  :::
...
- [Core Concepts](/guide/concepts/) — the vocabulary these lanes are built on
