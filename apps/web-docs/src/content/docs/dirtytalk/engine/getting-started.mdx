...
description: From an empty file to a working DirtyChannel you can mark and observe, then swapping the scheduler to change timing.
---
...
The package is ESM-first (`"type": "module"`), side-effect free, and has zero runtime dependencies. It ships a single entry point, `@dirtytalk/engine`.
