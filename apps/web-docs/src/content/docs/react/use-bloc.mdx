---
title: useBloc
description: The useBloc hook connects a React component to a state container, returning the current state, the bloc instance, and a ref.
---

import UseBlocDemo from '../../../components/demos/UseBlocDemo.tsx';
...
class UserCardCubit extends Cubit<UserCardState, { userId: string }> {
  ...
  // Side effects belong in onActivate, not init — see Lifecycle below.
...
3. **`onActivate(signal)` fires** on the instance's 0→1 ownership transition — i.e. the first time anything acquires it. Fired in a layout effect.
4. **Subscribe:** the hook subscribes to the bloc's channel using the selected tracking mode (auto-track or `select`)
5. **`onMount(bloc)` fires** in a mount effect, after the bloc is acquired
6. **Re-render:** only triggered when a tracked state path or a `select` value changes
7. **Unmount:** `onUnmount(bloc)` fires (bloc still alive), then `release(BlocClass)` decrements the ref count. At zero, `onDeactivate()` fires (aborting the `onActivate` signal) and the instance is disposed unless the class is [`keepAlive`](/core/configuration)

`init` remains the place to seed synchronous state from `args` — it is not deprecated. `onActivate`/`onDeactivate` are for side effects (fetch, subscribe, timers) tied to whether anything currently owns the instance; see [Cubit](/core/cubit#onactivate--ondeactivate) for the full model.

:::caution[Ordering with `useBlocDeps`]
`onActivate` fires in a layout effect, before the first `useBlocDeps` slice is applied (which happens in a passive effect). A bloc reading `this.deps.x` inside `onActivate` sees `undefined` on first activation — this matches today's `init()` timing, so it isn't a regression, but `onActivate` is the wrong hook for deps-driven work. Use `onDepsChanged` instead.
:::
...
`useBloc` re-renders when a tracked path (or the `select` value) changes, and not otherwise. Reads are consistent within a single render pass.
