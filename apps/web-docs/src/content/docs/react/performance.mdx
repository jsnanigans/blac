---
title: Performance
description: BlaC's re-render isolation — each component re-renders only when the state it reads changes — plus patterns for measuring it and rendering large lists.
---

import PerformanceDemo from '../../../components/demos/PerformanceDemo.tsx';
...
`CountButtons` never re-renders on count changes because it reads nothing from `state`.
