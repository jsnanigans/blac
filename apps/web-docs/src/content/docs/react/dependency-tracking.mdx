...
A value read from `state` is a tracking proxy, not a plain object. Passing one
down as a prop to a child that does not call `useBloc` itself works, but the
child's reads are attributed to the **parent** that created the proxy — the
parent is what re-renders when that path changes, and the parent's tracked set
grows to cover whatever the child read.

```tsx
function Parent() {
  const [state] = useBloc(ListBloc);
  // Row's reads of `item` land in Parent's tracked set.
  return <Row item={state.data[0]} />;
}
```

For a memoized child this defeats the memo: the child re-renders because the
parent did. Reads are also only recorded during render — after commit the proxy
tree is frozen and later reads (in effects, callbacks, or timeouts) pass through
untracked, so they never establish a dependency.

Have the child call `useBloc` itself, or pass plain values (`item.label`) rather
than the proxy, when you want the child to own its own dependencies.
