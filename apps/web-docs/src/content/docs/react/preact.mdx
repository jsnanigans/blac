---
title: Preact
description: The planned @blac/preact binding provides the same useBloc hook and API as @blac/react over the same @blac/core engine.
---
...
`configureBlacPreact`'s configuration surface is intentionally empty today. The hook's tracking model is fixed — auto-tracking when `select` is omitted, selector-driven re-renders when it is provided — and is not configurable.
