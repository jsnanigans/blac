/**
 * Sandpack project files for the tutorial's two interactive checkpoints.
 *
 * Every export is a plain string (no runtime imports), so this module is
 * SSR-safe and can be imported from an `.mdx` page at the top level. The maps
 * are handed to `<BlacSandpack files={...} />`, which mounts them client-only
 * and resolves the PUBLISHED `@blac/*` from Sandpack's CDN.
 *
 * The source mirrors the tutorial prose exactly:
 *   - `tutorialInteractiveFiles` — the Step-4 checkpoint (add / toggle / remove
 *     / filter + derived `remaining`).
 *   - `tutorialTimeTravelFiles` — the Step-7 finale (a `past`/`cursor` undo
 *     stack with undo / redo / jump-to-snapshot).
 *
 * Uses the real published API surface: `Cubit` from `@blac/core`, `useBloc`
 * from `@blac/react` (returns `[state, bloc]`).
 */

/* ------------------------------------------------------------------ */
/* Checkpoint A — Step 4: the interactive Todo app                      */
/* ------------------------------------------------------------------ */

const interactiveCubit = `import { Cubit } from '@blac/core';
...
const interactiveApp = `import { useState, KeyboardEvent } from 'react';
...
/* ------------------------------------------------------------------ */
/* Checkpoint B — Step 7: undo / redo / time-travel                     */
/* ------------------------------------------------------------------ */

const timeTravelCubit = `import { Cubit } from '@blac/core';
...
const timeTravelApp = `import { useState, KeyboardEvent } from 'react';
import { useBloc } from '@blac/react';
import { TodoCubit, Filter } from './TodoCubit';
import './styles.css';
...
/** Step-4 checkpoint project. Keys are absolute sandbox paths. */
export const tutorialInteractiveFiles: Record<string, string> = {
  '/App.tsx': interactiveApp,
  '/TodoCubit.ts': interactiveCubit,
  '/styles.css': baseStyles,
};

/** Step-7 (time-travel) checkpoint project. Keys are absolute sandbox paths. */
export const tutorialTimeTravelFiles: Record<string, string> = {
  '/App.tsx': timeTravelApp,
  '/TodoCubit.ts': timeTravelCubit,
  '/styles.css': baseStyles,
};
