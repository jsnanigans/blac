...
::: warning Map / Set / Date / class instances are leaves
...
::: details "A new instance is created on every render"
...
::: details How do I track a value without decorating it? Where is `@tracked`?
...
::: details `emit` vs `update` vs `patch`?
...
::: details Why did my `select` start re-keying / re-subscribing each render?
...
::: details My tests leak state between cases.
...
::: details A circuit breaker threw ("max instances" / "max refs").
