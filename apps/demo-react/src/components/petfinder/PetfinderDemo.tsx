            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg overflow-hidden border border-purple-100 dark:border-purple-900/30">
              <div className="p-5 border-b border-gray-200 dark:border-gray-700">
                <h2 className="text-xl font-bold text-gray-800 dark:text-white flex items-center">
                  ...
                  Blac Implementation Details
                </h2>
              </div>
              <div className="p-5 space-y-5">
                <div>
                  <h3 className="text-md font-semibold text-purple-600 dark:text-purple-400 mb-2">1. State Management</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    The <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded">PetfinderBloc</code> class extends Blac's <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded">Cubit</code> to manage application state with immutable updates. This creates a predictable flow of data with one source of truth.
                  </p>
                </div>
                
                <div>
                  <h3 className="text-md font-semibold text-purple-600 dark:text-purple-400 mb-2">2. Granular Loading States</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    Instead of a single loading flag, we use a <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded">LoadingState</code> object with separate properties for initial loading, pagination, and pet details. This provides a better UX by showing specific loading indicators.
                  </p>
                </div>
                
                <div>
                  <h3 className="text-md font-semibold text-purple-600 dark:text-purple-400 mb-2">3. Async Operations</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    All API calls are wrapped in async methods that handle their own loading states and errors. Each operation follows a pattern:
                  </p>
                  <ol className="list-decimal list-inside text-sm text-gray-700 dark:text-gray-300 mt-2 space-y-1">
                    <li>Set appropriate loading state</li>
                    <li>Try to perform the async operation</li>
                    <li>Update state with results</li>
                    <li>Handle errors gracefully</li>
                  </ol>
                </div>
                
                <div>
                  <h3 className="text-md font-semibold text-purple-600 dark:text-purple-400 mb-2">4. Component Integration</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    Components use the <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded">useBloc</code> hook to connect to the PetfinderBloc. This returns:
                  </p>
                  <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 mt-2 space-y-1">
                    <li>Current state: for rendering UI</li>
                    <li>Bloc instance: for triggering actions</li>
                  </ul>
                </div>
                
                <div>
                  <h3 className="text-md font-semibold text-purple-600 dark:text-purple-400 mb-2">5. Immutable Updates</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    The <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded">patch()</code> method creates partial updates to the state, while <code className="bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded">emit()</code> replaces the entire state. This pattern ensures predictable state transitions and avoids mutation bugs.
                  </p>
                </div>
              </div>
            </div>
