...
const UserSchema = z.object({
  ...
  age: z.number().int('Age must be a whole number').min(13, 'Must be at least 13').max(120),
...
export function SchemaValidationInteractive() {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {/* Current Test Data */}
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">Current Test Data (Before Validation)</h3>
      ...
      <motion.div
        ...
        className="space-y-3"
      >
        <h3 className="text-sm font-semibold text-foreground">Validation Result</h3>
          ...
          <div className="rounded-lg border border-red-500 bg-red-50 dark:bg-red-950/30 p-4">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-red-700 dark:text-red-400 mt-1">{state.lastError}</p>
                <p className="text-xs text-muted-foreground mt-2">State was NOT updated</p>
              </div>
            </div>
          </div>
        )}
        ...
        {!state.isSubmitting && !state.lastError && state.userData && (
          <div className="rounded-lg border border-green-500 bg-green-50 dark:bg-green-950/30 p-4">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-green-800 dark:text-green-300">Validation Passed!</p>
                <p className="text-xs text-green-700 dark:text-green-400 mt-1">State updated with validated data</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-foreground">Cubit State (Always Valid)</h3>
      ...
      {/* Helper Methods Demo */}
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">Helper Methods</h3>
        <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-3">
          <div className="text-xs space-y-2">
            <code className="bg-muted px-2 py-1 rounded font-mono">UserSchema.safeParse(data)</code>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
              <div className="flex items-center gap-2">
                <Badge variant="default" className="bg-green-600 text-white">Valid</Badge>
                <span className="text-xs text-muted-foreground">Returns success: true</span>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="border-red-500 text-red-700 dark:text-red-400">Invalid</Badge>
                <span className="text-xs text-muted-foreground">Returns success: false + errors</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-lg border border-brand/50 bg-brand/5 p-4">
        <p className="text-xs text-muted-foreground">
          <strong className="text-foreground">Key Insight:</strong> Schema validation prevents invalid data from ever entering your state.
          When validation fails, the state remains unchanged, guaranteeing data integrity throughout your application.
        </p>
      </div>
    </div>
  );
