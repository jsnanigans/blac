...
const initialState: TodoState = {
  todos: [
    ...
    { id: 2, text: 'Build an app with BlaC', completed: false },
...
// Register events for Redux DevTools action dispatch
// (Alternative to using @DevToolsEvent() decorator)
EventRegistry.register('AddTodoAction', AddTodoAction, {
  parameterNames: ['text'],
});

EventRegistry.register('ToggleTodoAction', ToggleTodoAction, {
  parameterNames: ['id'],
});

EventRegistry.register('RemoveTodoAction', RemoveTodoAction, {
  parameterNames: ['id'],
});

EventRegistry.register('SetFilterAction', SetFilterAction, {
  parameterNames: ['filter'],
});

EventRegistry.register('ClearCompletedAction', ClearCompletedAction);
