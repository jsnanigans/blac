...
function getSectionForDemo(demoId: string, explicitSectionId?: string) {
  if (explicitSectionId) {
    return guideStructure.sections.find((section) => section.id === explicitSectionId);
  }

  return guideStructure.sections.find((section) => section.demos.includes(demoId));
}
...
export function GuideArticle({ demoId, sectionId, children }: GuideArticleProps) {
  ...
  if (!demo) {
    return (
      <div className="rounded-lg border border-border bg-surface p-6 shadow-subtle">
        <h1 className="text-xl font-semibold text-foreground">Demo not found</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          The guide entry could not be located. Please check the URL or pick another demo from the
          navigation.
        </p>
      </div>
    );
  ...
  const description = demo.description ?? 'This demo does not yet have a description.';
    ...
    <article className="space-y-10">
      ...
      {demo.relatedDemos && demo.relatedDemos.length > 0 && (
        <section className="space-y-4 rounded-lg border border-border bg-surface p-6 shadow-subtle">
          <h2 className="text-lg font-semibold text-foreground">Related demos</h2>
