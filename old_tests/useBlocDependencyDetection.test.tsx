...
describe('useBloc dependency detection', () => {
      ...
      return (
        <div>
          ...
          <button
            ...
            onClick={() => {
              cubit.updateName('New Name' + Math.random());
            }}
          >
            Update Name
          </button>
  ...
  test('should detect dependencies in nested objects', async () => {
      ...
      return (
        <div>
          ...
          <button
            ...
            onClick={() => {
              cubit.updateDeepProperty('Updated Deep Property');
            }}
          >
    ...
    expect(screen.getByTestId('deep-property')).toHaveTextContent(
      'deep property',
    );
    ...
    const DynamicComponent: FC = () => {
      ...
      return (
        <div>
          ...
          {state.flags.showName && <div data-testid="name">{state.name}</div>}
  ...
  test('should respect custom dependency selector', async () => {
    ...
    const CustomSelectorComponent: FC = () => {
      ...
      const [state, { increment, updateName }] = useBloc(CustomSelectorBloc, {
        selector: (currentState, previousState, instance) => [
          currentState.count, // Only depend on count
        ],
      });
      renderCount++;
    ...
    expect(renderCount).toBe(1); // Should NOT re-render because custom selector doesn't depend on name
    expect(screen.getByTestId('name')).toHaveTextContent('Initial Name'); // UI won't update unless count changes

    // Update name again (NOT in custom selector) - should still NOT re-render
    await userEvent.click(screen.getByText('Update Name'));
    ...
    expect(renderCount).toBe(2); // NOW it should re-render
    expect(screen.getByTestId('count')).toHaveTextContent('1');
      ...
      return (
        <div>
          ...
          <button
            ...
            onClick={() => {
              cubit.updateName('Updated Shared Name');
            }}
          >
            Update Name
          </button>
  ...
  test('should correctly track dependencies in conditional renders', async () => {
      ...
      return (
        <div>
          ...
          <button
            data-testid="update-name"
            onClick={() => {
              cubit.updateName('Updated Conditionally');
            }}
          >
            Update Name
          </button>
    ...
    expect(screen.getByTestId('conditional-name')).toHaveTextContent(
    ...
    await act(async () => {
      await userEvent.click(screen.getByTestId('update-name'));
    }); // Name becomes "Updated Conditionally" again, but triggers render
    expect(screen.getByTestId('conditional-name')).toHaveTextContent(
      'Updated Conditionally',
    );
