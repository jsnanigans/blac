...
const BlocItem: FC<{
  bloc: BlocBase<any>, onSelect: (selected: BlocBase<any>, event?: ChangeEvent<any>) => void, selected: boolean,
}> = (
  {
    bloc,
    onSelect,
  ...
  const addChange = useCallback((e: ChangeEvent<any>) => {
    setEvent(e);
...
const BlocDetails: FC<{ bloc?: BlocBase<any>, lastEvent?: ChangeEvent<any> }> = (
  { bloc, lastEvent }
) => {
  ...
  const addChange = useCallback((event?: ChangeEvent<any>) => {
    setEvent(event);
  }, [bloc]);
  ...
  return <div className={classes.details}>
    {selected && <div>
      ...
      <Paper className={classes.value}>
        <pre className={classes.code}>{JSON.stringify(selected.state, null, 2)}</pre>
      </Paper>

      <Typography variant="h5">Most recent change</Typography>
      <Paper className={classes.value}>
        {!event && <Alert severity="info">No events captured</Alert>}
        {event && <Grid container spacing={3}>
          <Grid item sm={6}>
            <Typography variant="body1">From</Typography>
            <pre className={classes.code}>{JSON.stringify(event?.currentState, null, 2)}</pre>
          </Grid>
          <Grid item sm={6}>
            <Typography variant="body1">To</Typography>
            <pre className={classes.code}>{JSON.stringify(event?.nextState, null, 2)}</pre>
          </Grid>
        </Grid>}
      </Paper>
        ...
        <TableContainer>
          <Table>
            <TableBody>
              ...
              <TableRow>
                <TableCell><strong>Register Listeners</strong></TableCell>
                <TableCell>{selected.registerListeners.length}</TableCell>
              </TableRow>
              <TableRow>
                <TableCell><strong>Change Listeners</strong> (2 are from the debug tools)</TableCell>
                <TableCell>{selected.changeListeners.length}</TableCell>
              </TableRow>
              <TableRow>
                <TableCell><strong>Value Change Listeners</strong></TableCell>
                <TableCell>{selected.valueChangeListeners.length}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </TableContainer>
