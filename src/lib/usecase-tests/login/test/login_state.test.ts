...
describe("LoginState", function() {
  ...
  it("should copy", function() {
    ...
    let clone = loginState.copyWith(new User("hello, i noticed", "couple bugs"), new Password("so"));
  ...
  it("should copy without loss", () => {
    ...
    expect(clone.password === loginState.password && loginState.user !== clone.user).toBe(true)
  })


  function createLoginState() : LoginState{
    return new LoginState( new User(
        "and btw",
        "blac is awesome"
      ),
      new Password(
